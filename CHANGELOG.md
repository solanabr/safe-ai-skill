...
First stable release. Ships the ssai engine, runtime action firewall, and supply-chain verifier as a standalone Claude Code plugin targeting `solanabr/solana-ai-kit` v2.0.0 as its primary integration hub.

### Engine

- Static Rust binary (`ssai` / `safe-solana-ai`) with synchronous I/O; no runtime dependency. Prebuilt for `darwin-arm64`, `darwin-x64`, `linux-x64` with `SHA256SUMS`. Falls back to `cargo build --release` if no prebuilt matches; shim exits code 2 (block) if neither path is available — never fails open.
...
- Policy DSL (`default.policy.yaml`): full schema synced on-disk including `catalog`, `ext`, and `exec_install_scripts` fields. Deep-merge over project `.safe-solana-ai/policy.yaml`. Fail-closed on parse error.
...
- Per-`ext`-submodule verification (`ext_verify.rs`): each of solana-ai-kit's 18 `ext/` git submodules is walked independently at `SessionStart`. Git SHA pinned on first seen (TOFU). SHA drift triggers quarantine + diff in `additionalContext` + `reloadSkills: true`. `ssai verify approve ext/<name>` re-pins after user review.
...
- `add mcp <id>` resolves catalog MCP entries, pins to exact `pkg@version` + `dist.shasum`, writes to `.mcp.json`. `@latest` entries in existing `.mcp.json` are flagged INFORMATIONAL — not auto-rewritten. `ssai pin-mcps` offered as opt-in rewrite.
...
- solana-ai-kit ships all 7 MCP servers at `@latest`. ssai flags these INFORMATIONAL (LOW) at `install` time and `SessionStart` by design — the kit's `@latest` posture is intentional, and ssai does not nag or auto-pin. Runtime gating by tool name and payload operates independently of pin status.

### Relaxation model

- Time-boxed grants (`ssai allow`): relax soft gates for a bounded scope and duration. Stored in `grants.json`; expire automatically. `ssai revoke` cancels early.
- Hard guards bypass no grant, profile, or flag in v1. This is an engine property, not a default.

### Distribution

- Standalone plugin (`claude plugin marketplace add solanabr/safe-solana-ai`). Pre-enabled in repo `.claude/settings.json` for contributors.
- CI builds all three platform binaries and regenerates `SHA256SUMS` on every release tag.
- Folding ssai into the `stbr` solana-ai-kit marketplace is deferred to a future release.
...
- `ssai doctor` dual-install/coexistence detection (P2).
