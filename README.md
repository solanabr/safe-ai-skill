...
Install `solanabr/solana-ai-kit` (the config hub: agents, commands, rules, skills, catalog) and enable ssai as a plugin (the hooks-only security layer). No `settings.json` merge is needed — Claude Code merges plugin hooks with project hooks automatically, and ssai's `deny` decisions survive even `enableAllProjectMcpServers: true` and `bypassPermissions`. The kit's broken `read -r` mainnet gate is superseded by ssai's `ask` gate without any modification to the kit's settings.

ssai is standalone — anyone can install it independently of solana-ai-kit. Bundling ssai into the `stbr` marketplace is a future release.
...
```bash
safe-solana-ai registry list          # list catalog entries with risk classification
safe-solana-ai registry verify        # audit installed entries against registry
safe-solana-ai add skill <name|url>   # catalog entry or any GitHub URL, verified before install
safe-solana-ai add mcp <id|pkg|url>   # any MCP; verified before writing .mcp.json
safe-solana-ai verify                 # on-demand audit of all installed skills and MCPs
safe-solana-ai status                 # pins, quarantine list, active profile, live grants, recent decisions
```

Each `add` runs the intrinsic verification pipeline on the actual fetched content before executing the underlying install command. `registry list` shows the full 39-entry opt-in catalog with high-risk classes flagged. High-risk entries (`wallet_signing`, `key_custody`, `installer_script`) require explicit approval; ssai blocks installation of denied classes and gates execution of approved ones at runtime.

### Tier 2 — verified hub install

```bash
safe-solana-ai install                        # hub-agnostic verified install of solana-ai-kit
safe-solana-ai install --from <url>           # install from a specific source URL or GitHub ref
safe-solana-ai install --home ~/.claude       # override install destination
```
...
3. **Catalog gating** — entries in `skill-registry.json` are classified by risk class (`wallet_signing`, `key_custody`, `installer_script`, `standard`). High-risk classes require explicit policy approval; ssai blocks installation of denied classes without prompting.
4. **Provenance pinning** — GitHub URLs are resolved to immutable commit SHAs (moving refs are rejected). npm packages are pinned to `pkg@x.y.z` + `dist.shasum`. `@latest` MCP entries are flagged informational — ssai does not silently rewrite them.
