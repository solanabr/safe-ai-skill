...
The `safe-ai-skill` CLI and its `safe-ai-skill` alias are the same binary. `safe-ai-skill` is used where brevity matters (hook invocations, session commands); `safe-ai-skill` is the canonical user-facing form.

---

### `safe-ai-skill install`
...
safe-ai-skill install --from https://github.com/solanabr/solana-ai-kit@a3c3d23

# Override the install destination (default: ./.claude)
safe-ai-skill install --home ~/.claude
```
...
Scanning .mcp.json (7 MCPs)...
  INFO  helius-mcp:              @latest (informational — use 'safe-ai-skill pin-mcps' to pin)
...
### `safe-ai-skill add skill <name|url>`
...
**High-risk entry gating.** If `<name>` resolves to a `wallet_signing` or `key_custody` catalog entry, safe-ai-skill shows a risk summary and requires explicit confirmation — regardless of the general policy setting — before proceeding:

```
safe-ai-skill add skill phantom-mcp
...
# To allow, update .safe-ai-skill/policy.yaml:
#   catalog:
...
safe-ai-skill add skill anchor-specialist

# Install from an arbitrary GitHub URL
safe-ai-skill add skill https://github.com/example/my-solana-skill

# Install from a specific commit (already immutable)
safe-ai-skill add skill github:example/my-solana-skill@abc1234
```

---

### `safe-ai-skill add mcp <id|pkg|url>`
...
`@latest` entries already in `.mcp.json` (from a kit install) are **not** auto-rewritten. safe-ai-skill flags them as INFORMATIONAL and offers `safe-ai-skill pin-mcps` as an opt-in rewrite.
...
```
safe-ai-skill add mcp x402-proxy-mcp
...
safe-ai-skill add mcp helius

# Install by npm package name (resolves to exact version)
safe-ai-skill add mcp @modelcontextprotocol/server-filesystem

# Install a specific version (still fully verified)
safe-ai-skill add mcp @modelcontextprotocol/server-filesystem@1.2.3
```

---

### `safe-ai-skill add repo <url>`
...
```bash
# Rejected — moving ref
safe-ai-skill add repo https://github.com/example/tools

# Accepted — pinned to commit SHA
safe-ai-skill add repo https://github.com/example/tools@a3f9c12

# Also accepted — fragment syntax
safe-ai-skill add repo https://github.com/example/tools#a3f9c12de4b5f6789012345678901234567890ab
```
...
# MCPs:
#   INFO @helius-labs/helius-mcp  @latest (informational; use 'safe-ai-skill pin-mcps' to pin)
#   OK   @modelcontextprotocol/server-filesystem@1.2.3
# Run 'safe-ai-skill verify approve <name>' to re-pin after review.
```
...
```bash
safe-ai-skill mode autopilot   # raise caps for a high-frequency devnet session
safe-ai-skill mode strict      # restore defaults
safe-ai-skill mode off         # disable soft gates (hard guards remain)
```
...
safe-ai-skill allow --scope mainnet_transfer --for 30m --max-tx 2.0

# Allow devnet deploys to a specific program for 2 hours
safe-ai-skill allow --scope devnet_deploy --for 2h --programs TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA

# Allow transfers to a specific address up to a 5 SOL budget today
safe-ai-skill allow --scope mainnet_transfer --for 8h --to 9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM --budget 5.0
```

Grants are stored in `${CLAUDE_PLUGIN_DATA}/grants.json` and expire automatically. `safe-ai-skill status` shows all active grants with remaining time.

---

### `safe-ai-skill revoke`
...
# Revoke by grant ID (shown in safe-ai-skill status output)
safe-ai-skill revoke grant_a1b2c3

# Revoke all active grants
safe-ai-skill revoke --all
```

---

### `safe-ai-skill session init --cap <SOL>`
...
```bash
safe-ai-skill session init --cap 0.5
...
### `safe-ai-skill verify approve <name>`
...
safe-ai-skill verify approve

# Restore and re-pin a specific ext/ submodule after reviewing the diff
safe-ai-skill verify approve ext/solana-new

# Restore a skill dir
safe-ai-skill verify approve anchor-specialist
```
...
Place a file at `<project>/.safe-ai-skill/policy.yaml` to override the defaults for that project. The file is deep-merged over the defaults: only keys present in the override file take effect. Array values replace (not extend) the corresponding default arrays.

```yaml
# <project>/.safe-ai-skill/policy.yaml
...
```
safe-ai-skill: MAINNET DEPLOY — approve to proceed?
...
```
safe-ai-skill: Transfer of 5.0 SOL exceeds per-transaction cap (1.0 SOL). Approve?
...
```
safe-ai-skill: MCP value-moving call — approve?
...
```
safe-ai-skill: HIGH-RISK MCP — wallet_signing class. Approve?
...
```
safe-ai-skill: Install script detected — approve execution?
Pattern: curl ... | bash
...
```
safe-ai-skill: Install script execution denied
Pattern: curl ... | bash
...
```
safe-ai-skill: Rugcheck unavailable — approve swap manually?
Mint: Hf...xQ
...
```
safe-ai-skill: Outbound POST denied
...
```
[safe-ai-skill] Supply-chain warning — 1 ext/ submodule quarantined:
...
To review and restore:
  safe-ai-skill verify           # see what changed
  safe-ai-skill verify approve ext/solana-new   # re-pin after review
```

Run `safe-ai-skill status` to see the full quarantine list and all flagged findings from the current session's scan.
