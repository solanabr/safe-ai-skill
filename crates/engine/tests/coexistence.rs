//! safe-ai-skill-as-plugin coexistence with the solana-ai-kit host — engine-decision backing.
//!
//! safe-ai-skill ships as a standalone Claude Code plugin alongside solana-ai-kit. The coexistence
//! claims (plugin hooks merge with the kit's project hooks; most-restrictive-wins; safe-ai-skill's
...
//!   - the kit's broken mainnet `read -r` gate is superseded: safe-ai-skill's `gate-bash` asks on a
...
fn curl_pipe_bash_install_script_asks() {
    ...
    // restrictive decision a merged safe-ai-skill plugin hook contributes when the kit's agents try to
...
fn mainnet_deploy_asks_superseding_kit_read_gate() {
    ...
    // (no TTY). safe-ai-skill's `gate-bash` supersedes it: a mainnet deploy asks outright, with no prompt.
    assert_eq!(
        gate(
            "gate-bash",
...
fn devnet_deploy_allows_no_spurious_gate() {
    // Coexistence must not over-restrict: a devnet deploy is allowed, so safe-ai-skill does not block the
...
fn solana_transfer_still_gated_under_coexistence() {
    // The kit keeps `Bash(solana transfer *)` as a defense-in-depth deny; safe-ai-skill's own gate asks
    // on an over-cap transfer. Either way the action is not silently allowed — assert safe-ai-skill's
